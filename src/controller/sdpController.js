module.exports = (container) => {
    ...
    const { campaignRepo } = container.resolve('repo')
    const addCampaign = async (req, res) => {
        try {
            ...
            const {
                error,
                value
            } = await schemaValidator(body, 'Campaign')
            if (error) {
                return res.status(httpCode.BAD_REQUEST).json({ msg: error.message })
            }
            const item = await campaignRepo.addCampaign(value)
    ...
    const getCampaignById = async (req, res) => {
        try {
            const { id } = req.params
            if (id && id.length === 24) {
                const item = await campaignRepo.getCampaignById(id)
    ...
    const updateCampaign = async (req, res) => {
        try {
            ...
            if (id && id.length === 24 && body) {
                ...
                const item = await campaignRepo.updateCampaign(id, value)
                return res.status(httpCode.SUCCESS).json(item)
            }
    ...
    const getCampaign = async (req, res) => {
        try {
            ...
            Object.keys(search).forEach(i => {
                ...
                const pathType = (Campaign.schema.path(i) || {}).instance || ''
            ...
            const data = await campaignRepo.getCampaign(pipe, perPage, skip, sort)
            const total = await campaignRepo.getCount(pipe)
